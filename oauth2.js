 ...
 */
 ...
 Strategy.prototype.userProfile = function(accessToken, done) {
     let newOauth2 = this._oauth2
     let newAccess = accessToken
     this._oauth2.get("https://id.twitch.tv/oauth2/validate", accessToken, function (err, body, res) {
         if (err) { return done(new InternalOAuthError("failed to fetch user profile", err)); }
 
         try {
             var json = JSON.parse(body);
 
             var profile = { provider: "twitch",id:json.client_id, login:json.login, accessToken };
